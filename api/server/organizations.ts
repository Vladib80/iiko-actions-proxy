...
import { makeHttp, ensureBridgeKeyNode } from '../../lib/http.ts';

async function getToken(base: string, login: string, password: string) {
  const http = makeHttp();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const orgs = arr.map((o: any) => ({ id: o.id || o.organizationId || o.guid, name: o.name || o.title }));
