...
import { makeHttp, ensureBridgeKeyNode } from "../../lib/http.js"; // <-- 2 levels up
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let r = await http.get(`${base}/api/0/organization/list`, {
      params: { access_token: token, request_timeout: 30000 }
    });

    // Fallback (some servers expose it under /resto/api)
    if (r.status !== 200) {
      r = await http.get(`${base}/resto/api/organization/list`, {
        params: { access_token: token, request_timeout: 30000 }
      });
    }
    ...
    if (r.status !== 200) {
      return res.status(r.status).json({
        error: true, code: "SERVER_ORGS_FAILED", message: "Org list failed",
        details: String((r.data && JSON.stringify(r.data).slice(0,200)) || r.status)
      });
    }
